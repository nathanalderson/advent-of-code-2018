...
case class AStar[State, Command](
  ...
  private val costSoFar = mutable.Map(start -> 0.0)
  ...
  def computePath: Option[List[Command]] = {
    ...
    while (open.nonEmpty) {
      ...
      for (
        ...
        val new_cost = costSoFar(current) + cost(current, neighbour)
        if (costSoFar.get(neighbour).map(new_cost < _).getOrElse(true)) {
          costSoFar.update(neighbour, new_cost)
